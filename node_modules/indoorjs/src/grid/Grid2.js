...
class Grid extends Base {
  ...
  // re-evaluate lines, calc options for renderer
  update2(center) {
    const shape = [this.canvas.width / center.zoom, this.canvas.height / center.zoom];
  ...
  setZoom(zoom) {
    ...
    this.axisX.zoom = 1 / zoom;

    this.axisY.zoom = 1 / zoom;
  }
  ...
  drawLines(state) {
    ...
    // draw ticks
    if (ticks.length) {
      const lineWidth = state.axisWidth / 2;
      // ctx.beginPath();
  ...
  drawLabels(state) {
    if (state.labels) {
      ...
      // const font = `300 ${state.fontSize}px ${state.fontFamily}`;
      const fill = state.labelColor || 'black';
